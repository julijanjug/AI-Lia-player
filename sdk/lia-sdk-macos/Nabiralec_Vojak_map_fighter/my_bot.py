...
def GetState_Vojak(api, unit):
    ...
    if len(unit["opponentsInView"]) > 0:
        ...
        closestOpponent = unit["opponentsInView"][0]
        distanceToOpponent = math_util.distance(unit["x"], unit["y"], closestOpponent["x"], closestOpponent["y"])

        for oppo in unit["opponentsInView"]:
            if math_util.distance(unit["x"], unit["y"], oppo["x"], oppo["y"]) < distanceToOpponent:
                closestOpponent = oppo
                distanceToOpponent = math_util.distance(unit["x"], unit["y"], closestOpponent["x"], closestOpponent["y"])

        orientationToOpponent = min(math_util.angle_between_unit_and_point(unit, closestOpponent["x"], closestOpponent["y"]), 30)
        distanceToOpponent = math_util.distance(unit["x"], unit["y"], closestOpponent["x"], closestOpponent["y"])
    else:
        canSeeOpponent = 0
        orientationToOpponent = 31
        distanceToOpponent = 31
    ...
    return speed, rotation, canSeeOpponent, orientationToOpponent, distanceToOpponent, distanceToObsticle
...
def GetReward_Vojak(api, unit, stateOfUnit, state):
    reward = -(int(state["numberOfOpponentUnits"])**2) * 10
    return reward
...
def minority_unit_type(state, minWorkers):
    workers = 0
    warriors = 0
...
class MyBot(Bot):
    ...
    def update(self, state, api):
        ...
        opponentsLeft = int(state["numberOfOpponentUnits"])
        ...
        if state["resources"] >= constants.WORKER_PRICE:
            minority_type = minority_unit_type(state, minWorkers=3)
            #api.spawn_unit(UnitType.WARRIOR)
            #api.spawn_unit(UnitType.WORKER)
            api.spawn_unit(minority_type)
        ...
        for unit in state["units"]:
            ...
            if unit["type"] == "WARRIOR":
                stateOfUnit = GetState_Vojak(api, unit)
                reward = GetReward_Vojak(api, unit, stateOfUnit, state)
...
if __name__ == "__main__":
    ...
    VOJAK_DISCRETE_OS_SIZE = [                  3,      3,      2,      60+1+1,      30+1+1, 25]

    VOJAK_OBSERVATION_SPACE_LOW = np.array([    -1,     -1,     0,      -30,         0,      0])
    VOJAK_OBSERVATION_SPACE_HIGH = np.array([   1+1,    1+1,    1+1,    30+1+1,      30+1+1, 50+1])
    ...
    # distance to food
    # distance to obsticle
    NABIRALEC_DISCRETE_OS_SIZE =                [1,      3,     2,         60+1+1,     30+1+1,     25]

    NABIRALEC_OBSERVATION_SPACE_LOW = np.array( [0,     -1,     0,         -30,        0,          0])
    ...
    SaveIfBetter(VOJAK_q_table, "VOJAK", 1000-opponentsLeft)
